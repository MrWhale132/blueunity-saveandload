...
public class SaveAndLoadCodeGenWindow : EditorWindow
{
    ...
    public void ValidateState()
    {
        if (_codegenTargets.IsNotNullAndNotEmpty())
        {
            for (int i = _codegenTargets.Count - 1; i >= 0; i--)
            {
                ...
                {
                    BlueDebug.Debug(_codegenTargets[i]._assemblyQualifiedTypeName + " is not valid anymore. Removing from the list.");
    ...
    public IEnumerable<Type> FindTypesInFile(string filePath)
    {
        ...
        foreach (var typeReport in inspectionReport.TypeReports)
        {
            ...
            if (type == null)
            {
                Debug.LogError($"Could not resolve type {typeName} in namespace '{namespaceName}' from assembly '{assemblyName}'. " +
                    $"One reason this error can happen is when a generated type is generated for the first time, " +
                    $"changes to the file were detected but the newly generated type is not yet part of the domain.");
                continue;
            }
            ...
            foreach ((var logicalType, var generationResult) in typesAndTheirgenerationResults)
            {
                ...
                if (_userSettings.GenerateSaveHandlersAsNestedClassesInsideHandledType && BlueTools.HasEditableSourceFile(type, out var path))
                {
...

                    {
                        ...
                        if (Roslyn.CodeGen.HasRegionDefined(path, type, "SaveAndLoad AutoGenerated", Debug.Log, out int regionStartLine, out int regionEndLine))
                        {
                            ...
                            int i = regionStartLine;

                            while (i < regionEndLine)
                            {
                                var line = alteredLines[i];
                            ...
                            bool hasOldCode = i < regionEndLine;
                            ...
                            else
                            {
                                ...
                                startIndex = regionEndLine;
                                endIndex = regionEndLine;
                            }
